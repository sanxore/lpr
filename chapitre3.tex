...
\newcommand{\reels}{\mathbb{R}}
	...
	La collecte et la préparation des données est un processus extrêmement lent et complexe, traditionnellement fait à la main et dont l'automatisation est souvent très difficile.
			\begin{wrapfigure}[13]{c}{8.5 cm}
					    		
					    		\fbox{\includegraphics[scale=0.4]{charplot}}
					    		\caption{Représentation de l'information transportée par une page .pdf contenant un tableau pour une résolution d'image de 700x800.}
					    		\label{fig:charplot}
	...
	\item La liste des objets(texte, figure, tableau, etc.) contenus,
	\item La géométrie des figures contenues,
	\item Si la page est en une seule colonne ou plusieurs, \item etc.
	\end{itemize}
	\par
	Tout ce qu'un fichier .pdf indique est l'emplacement des caractères. La figure \ref{fig:charplot} ci-contre montre comment les caractères sont disposés sur la page de la figure \ref{fig:IFA-PDF}.
	\par Une solution existe : \textbf{Tabula} de \textit{Mozilla} écrite en langage \textbf{Ruby} et précédemment utilisée par nos prédécesseurs (\cite{CHEMLAL},\cite{NACER}). Mais celle-ci présente l'inconvénient de nécessiter de l'utilisateur de dessiner des rectangles autour des tables destinations contenues dans les .pdf, ce qui n'offre aucun avantage d'automatisation.
	\paragraph{}
	Notre solution utilise le package \textbf{pdfminer}\cite{pdfminer} du langage \textbf{Python} qui extrait les objets	\begin{wrapfigure}[16]{c}{8.5 cm}
			\centering			    		
			\fbox{\includegraphics[scale=0.4]{classplot}}
			\caption{Classification de l'information transportée par une page .pdf contenant un tableau selon ($\in$ zone de texte, $\notin$ zone de texte)}
			\label{fig:charclass}
		\end{wrapfigure}
	...
	\begin{enumerate}
	\item \textbf{Une phase de structuration des données:
	\begin{itemize}
	\item Extraire 
	\end{itemize}
	}
	
	\item \textbf{Une phase d'unification des modèles de données:}
	\end{enumerate}
		\begin{figure}[h]
		    		\centering
	...
	\subsubsection{Unification des modèles de données}
