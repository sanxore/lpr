...
\section{Analyse de l’existant\cite{NACER}}
...
réunions quotidiennes sont indispensables. Non seulement le prix est déterminé par les
comportements stratégiques de l’OCP et sa vision de marché mais il est aussi lié à des facteurs
imprévisibles aussi bien politiques qu’environnementaux.
\par
L’objectif de ce stage est la fluidification de ce processus afin de faciliter la prise de décision
ou exactement la détermination des prix de toute nature.
\begin{figure}[H]
					\centering
					\fbox{\includegraphics[width=.9\textwidth]{ch2-images/ex_flow}}
					
					\label{fig:archi}
				\caption{Architecture Globale.}
			\end{figure}
\par
Le processus est déclenché par le besoin d’information, après vient la recherche du mail relatif aux besoins de l’analyste. Ce besoin est décrit par l’organisme, le marché et le type de la publication. Normalement les fichiers sont stockés en une structure hiérarchique bien définie. En cas de manque d’information ou de fichier, la recherche se fait sur les mails.
\par
En se basant sur l’utilisation actuelle des fichiers, nous avons constaté que les analystes passent un temps important à structurer les données avant qu’ils les utilisent pour la négociation des prix de vente ou l’élaboration des structures de coût des produits (DAP, MAP ...) liées au prix des matières premières utilisées lors du processus de transformation.
\par
En effet, les structures de coût sont utilisées pour connaitre la marge de bénéfice sur ces produits pour ensuite adapter les prix de vente ou viser les marchés à fort gain.
\par
Après avoir structuré les données, l’analyste les transforme en information en les éditant sous Excel. Cette transformation, des fois manuelle, peut engendrer plusieurs erreurs qui sont liées à la protection de fichiers PDF ou leur mise en page. Enfin, il présente les résultats synthétisés sous forme d’une présentation Powerpoint.
\par
Après avoir présenté les résultats, l’accès à l’information devient pénible ou bien impossible dans le cas d’une perte, ce qui nécessite de reprendre le processus à nouveau avec les mêmes hypothèses. Cette contrainte mène généralement le processus à l’échec.
\par
Au vu de ce processus, nous avons pu identifier plusieurs problèmes parmi lesquels nous citons :
\begin{itemize}
	\item L’absence de la structuration automatique des fichiers sur la structure hiérarchique.
...
analyses.
	\item L’absence de la structuration automatique des données brutes.
	\item L’absence d’un outil d’extraction de données des fichiers PDF.
